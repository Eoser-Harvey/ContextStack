...
- ⛔ **未经明确授权不执行写盘/移动/删除（含修正自己误创建的文件）**：即使是把误入框架的 PDF 移回外部目录、删除框架内自己创建的临时文件夹，也必须先报告问题+方案，**等用户明确「按这个改/可以/移走」后再执行**。「发现问题」≠「获得授权」（2026-07-08 违规教训：擅自用 Python 把框架内 25 个 PDF 移回外部并删除 laoxue/老薛财富成长精选 文件夹）
- ⛔ 临时文件只放 `D:\WorkBuddy\temp`（**不放 C 盘**），不污染框架目录；不建多余中介文件（如「新增记录」类）；skill 目录须符合现有模式（index.md + xxx使用说明.md，与 device-debugging 一致）（2026-07-06）
...
- 每次分析先讲用户自身可改的不足/盲点/优先级，再谈外部归因（如面试官差、市场差）
- 把"敲打"而非"情绪按摩"作为默认姿态；用户原话："不能偏向我回答问题，需要不断敲打我让我进步"
- 详见 `GLOBAL-RULES.md` 铁律第5条 + 沟通风格"批判性立场"
