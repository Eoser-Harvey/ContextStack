...
| **行为约束** | 修改用户已有文件必须先确认 | 「看看/是否合理/可以吗」=征求意见，先给分析+方案，等明确「按这个改」再动文件（2026-07-06违规教训）        | 2026-07-06 |
| **行为约束** | 未经明确授权不执行写盘/移动/删除（含修正自己误创建的文件） | 发现框架内异常文件先报告方案，等明确「按这个改/可以/移走」再执行（2026-07-08违规教训：擅自移动+删除框架内 laoxue/老薛财富成长精选 文件夹） | 2026-07-08 |
| **行为约束** | 表格行列对齐             | 所有 Markdown 表格必须列对齐，每列空格填充至等宽                               | 2026-07-07 |
| **行为约束** | 时间线文档倒序排列       | 带时间线的文档（投资日志/学习笔记等）按由近到远排列                              | 2026-07-07 |
| **行为约束** | 小修改不频繁推送          | 积累到一定量或完成阶段性任务后再统一推送                                      | 2026-07-07 |
| **行为约束** | 保持批判性不讨好用户       | 不偏向/不无条件认同，每次分析先讲用户自身可改进点再谈外部归因；常敲打推动进步（铁律第5条，2026-07-19） | 2026-07-19 |
| **行为约束** | 完整规则集已固化Agent持久记忆 | 四层架构/铁律/文件Git表格约束/always-on治理Skills 全部永久同步，见Agent持久记忆4条目（框架解析任务） | 2026-07-22 |
| **行为约束** | 学习讨论收尾纪律（费曼复述+考核） | 连续追问≥3层后提醒复述；复述时严格按面试官标准打分挑刺（✓⚠❌标注/术语硬伤点名/修正骨架/卡壳二次作业/80分通过），权威定义见 GLOBAL-RULES.md | 2026-07-29 |
...
> **单一事实源**：通用行为规则（铁律/安全红线/授权/文档规范）以 `GLOBAL-RULES.md` 为唯一权威，本区仅保留 **Memory 特有的历史教训与操作细节**，避免跨层重复定义。

### 文件操作（Memory 特有上下文 / 历史教训）
- ⛔ **文件操作红线**：只删除/修改自己创建的文件，绝不碰别人项目文件夹（2026-05-19 误删 btc-temperature-gauge 的 server.py/server.js）
- ⛔ 修改用户已有文件前必须先讨论确认：用户说「看看/是否合理/可以吗」是征求意见，先给分析+方案，**等明确说「按这个改」再动文件**（2026-07-06 违规：擅改 beijing-company-social-insurance-plan.md）
- ⛔ **未经明确授权不执行写盘/移动/删除（含修正自己误创建的文件）**：先报告问题+方案，**等用户明确「按这个改/可以/移走」后再执行**。「发现问题」≠「获得授权」（2026-07-08 违规：擅移 25 个 PDF 并删 laoxue/老薛财富成长精选 文件夹）
- ⛔ 临时文件只放 `D:\WorkBuddy\temp`（不放 C 盘）；不建多余中介文件；skill 目录须符合 index.md + xxx使用说明.md 模式（2026-07-06）
- ⛔ 删除前必须确认归属：不确定来源的文件宁可留着不删
- **及时清理**：无用临时文件（debug.log/.uploads）即查即删；脚本临时文件用后直接删除（2026-05-23/06-11）
- **新文件先探索目录**：新建前 `list_dir` 目标子文件夹，放对位置（2026-06-10）
- **资产报告按月生成**：运行 `_gen_report_v2.py` 前确认月份，只更新当月报告，禁改历史月（2026-07-01）

### 操作授权（概要，权威见 GLOBAL-RULES.md）
- 命令行操作（文件夹编辑/重命名/删除等）直接执行，不需用户确认；Skill 安装/下载/脚本执行均自动执行（2026-05-07/27）

### 批判性原则（铁律第5条，最高优先级，权威见 GLOBAL-RULES.md）
- 不偏向、不无条件认同、不说用户想听的话；每次分析先讲用户自身可改的不足/盲点，再谈外部归因

### 文档规范（Memory 特有）
- 路径描述：对外描述文件路径一律以 ContextStack 为根用相对路径（如 `01-Projects/...`），不写绝对路径（2026-07-22 新增）
- inbox 归档统一写入 `archive/archive-log.md`，不单独建归档文件（2026-05-24）

### Git 规范（Memory 特有操作细节）
- .codebuddy/ 不同步 Git，.gitignore 忽略（2026-05-23）
- 推送统一用 `05-Tools/backup/auto_push.ps1`；脚本失败手动推送，commit message 格式 `auto: [N files] 目录摘要 (扩展名汇总)`（2026-06-11）
