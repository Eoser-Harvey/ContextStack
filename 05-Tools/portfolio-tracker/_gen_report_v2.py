...
L.append("- 跌破 **$50**（前低区）→ 进入低估区，不再恐慌卖出")
L.append("")
L.append("### 12.3 倒金字塔卖出（越涨卖越多，回测验证）\n")
L.append("**回测结论**（模拟 CRCL 涨到不同目标价）：")
L.append("- 涨到 $400 时：平均分配回收 ¥167.9万（**不够200万**）；倒金字塔回收 **¥204万（达成）**")
L.append("- 达成 200万 最低价：平均分配即使涨到 $2000 都达不到；**倒金字塔只需 $400**")
L.append("- **原因**：倒金字塔把更多股留到高位卖，均价更高（$303 vs $250）")
L.append("")
L.append("> 结论：**\"越涨卖越多\"优于\"平均分配\"**。以下两套路径均采用倒金字塔。")
L.append("")
L.append("### 12.4 200万路径（涨到 $450 达成清仓）\n")
L.append("| 档 | 触发价 | 卖出股数 | 单档回收(¥) | 累计(¥) | 剩余 |")
L.append("|----|--------|---------|-----------|---------|------|")
L.append("| 启动 | $95（站上MA120）| 100 | 63,897 | 63,897 | 900 |")
L.append("| 1 | $120 | 80 | 64,570 | 128,467 | 820 |")
L.append("| 2 | $180 | 120 | 145,282 | 273,748 | 700 |")
L.append("| 3 | $260 | 170 | 297,289 | 571,037 | 530 |")
L.append("| 4 | $350 | 230 | 541,443 | 1,112,480 | 300 |")
L.append("| 5 | $450 | 300 | 908,010 | **2,020,490** | 0 |")
L.append("")
L.append("**合计**：卖 1000 股，均价 $300，累计 **¥202.0万，达成 200万并清仓** ✅")
L.append("")
L.append("### 12.5 300万路径（涨到 $700 达成清仓）\n")
L.append("| 档 | 触发价 | 卖出股数 | 单档回收(¥) | 累计(¥) | 剩余 |")
L.append("|----|--------|---------|-----------|---------|------|")
L.append("| 启动 | $95（站上MA120）| 100 | 63,897 | 63,897 | 900 |")
L.append("| 1 | $180 | 80 | 96,854 | 160,751 | 820 |")
L.append("| 2 | $280 | 120 | 225,994 | 386,745 | 700 |")
L.append("| 3 | $400 | 170 | 457,368 | 844,113 | 530 |")
L.append("| 4 | $550 | 230 | 850,839 | 1,694,952 | 300 |")
L.append("| 5 | $700 | 300 | 1,412,460 | **3,107,412** | 0 |")
L.append("")
L.append("**合计**：卖 1000 股，均价 $462，累计 **¥310.7万，达成 300万并清仓** ✅")
L.append("")
L.append("### 12.6 执行纪律（比阶梯更重要）\n")
L.append("1. **到了触发价就卖，不因\"还能涨\"而违背**——藤本阶梯本意，UAMY 教训。")
L.append("2. **每档卖出后利润立即转出/记账**，不因看到又涨而贪心买回。")
L.append("3. **跌破 MA120 兜底独立于上行阶梯**：启动后若中途跌破 MA120 且收不回，暂停加仓；上行档位照常（两套独立）。")
L.append("4. **结合看板信号调整**：CRCL 信号分一旦进入低估区（<40），可上调后续档目标价；营收数据转差（基本面层走弱），提前兑现当前档。")
L.append("5. **目标达成即清仓**，不保留尾巴。")
L.append("")
L.append("### 12.7 关键风险提示（批判）\n")
L.append("1. **200万要 $450、300万要 $700**——远超稳定币龙头常规估值。成立前提：Circle 营收持续高增长 + 美债利率维持高位 + 市场给科技股估值，**三点缺一不可**。")
L.append("2. **分析师最高目标价 $175（仅 ATH 的 58%）**——连最乐观机构都不看回 $298，说明高目标依赖基本面兑现而非市场共识。")
L.append("3. **这不是\"还债计划\"，是\"高增长兑现计划\"**——90% 靠 Circle 基本面，10% 靠卖得够纪律。你能控制的只有那 10%。")
L.append("4. **若某季度营收增速大幅放缓，要果断下调目标价并提前兑现**，而非\"等它涨回来\"。")
L.append("")
L.append("*本节为个人纪律参考，不构成投资建议。CRCL 为高波动资产，请 DYOR、盈亏自负。*")
L.append("")
