...
```powershell
# Step 1: 在 CodeBuddy 登录新账户（让 Data 下生成新的 UUID 目录）
# Step 2: 完全关闭 CodeBuddy IDE
# Step 3: 用独立 PowerShell 窗口执行:
cd 05-Tools\codebuddy-chat-manager
.\chat-restore.ps1
# 按提示: 输入会话编号(如 1,3,5-8) -> 选择目标账户 -> 确认
```
...
3. **新账户需先登录一次**：让 CodeBuddy 生成新 UUID 目录，脚本才有写入目标。
