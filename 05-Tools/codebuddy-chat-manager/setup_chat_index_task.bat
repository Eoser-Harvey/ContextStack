...
:: Runs chat-index.ps1 every day at 21:50 (before auto_push at 22:00)
...
schtasks /create /f /tn "%TASK%" /tr "powershell.exe -ExecutionPolicy Bypass -File ""%SCRIPT%""" /sc daily /st 21:50 /rl highest

if %errorlevel% equ 0 (
    echo.
    echo [OK] Task "%TASK%" registered successfully.
    echo It will run chat-index.ps1 daily at 21:50 to refresh chat-index.md.
    echo.
