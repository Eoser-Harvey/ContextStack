...
.extern VAR_NAME(g_ptCurrentTcb);
...
.extern	VAR_NAME(g_uiOSRescheduleIntID);
 ...
 */
.extern	VAR_NAME(g_uiOSWaitingForSched);
...
.global FUNC_NAME(OsStartFirstTask);
...
FUNC_NAME(OsStartFirstTask):
	...
	// 将系统SP重置为系统堆栈的底部，因为它将仅用于从此点开始的中断和异常，因此堆栈上当前使用的所有空间都将被浪费
	 
	/* 
		 We are currently in the system (foreground) data and DAG regs
		 Data Memory (DM) 
	*/
	// Get the location of the current TCB into I0'
	I0 = DM(VAR_NAME(g_ptCurrentTcb)); 
	...
	*/
    ...
    /* PEx、PEy条件(ALU = 0 or exclusive access successful独立访问成功)都成立，进行Jmup.
     AZ=1代表EQ条件成立，AZ表示：ALU result zero or floating-point underflow
    */
...
.Initial_Loop_Stack:                             /* top of loop */
    /* 
    PUSH LOOP指令仅通过更改指针来压入堆栈。它不会更改循环地址堆栈的内容。
              因此，通常应在PUSH LOOP指令之后写入REGF_LADDR寄存器。
    */
    PUSH LOOP;
...
FUNC_NAME(_adi_SoftIntTaskSw):
       ...
       * already saved some register to the foreground (i.e.e system) stack. In order to mimic模拟 the
...
xPortSFT31Handler:
	...
	R14 = DM(VAR_NAME(g_ptCurrentTcb)); // R14 is a preserved reg	
	...
	R15 = DM(VAR_NAME(g_ptCurrentTcb));
	
	//R15 = DM(VAR_NAME(g_ptReadyTcb));
	...
	R0 = 0;
	DM(VAR_NAME(g_uiOSWaitingForSched)) = R0;
	...
	R4 = DM(VAR_NAME(g_uiOSRescheduleIntID));
.intDisable2:
 ...
 *************************************************/
	...
	I0 = DM(VAR_NAME(g_ptCurrentTcb)); // Get the location of the current TCB into I0'
	
	//I0 = DM(VAR_NAME(g_ptReadyTcb));
