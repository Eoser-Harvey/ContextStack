# 知识库 (02-Knowledge)

ContextStack 的知识管理中心，存放规范文档、技能模块和知识笔记。

## 目录

| 目录 | 内容 |
|------|------|
| [system/](./system/) | 系统规范文档 — 方法论、SOP、模板、调研框架、工具配置 |
| [skills/](./skills/) | 可复用技能模块 |
| [career-development/](./career-development/) | 职业发展知识库 |
| [inbox/](./inbox/) | 每日收件箱 — 未消化内容暂存区 |

## Inbox 今日新增（2026-05-30）

| 文件 | 分类 | 来源 |
|------|------|------|
| `wealth-game-golden-rules-budongjing.md` | 财富认知 | 微信公众号: 不懂经 |
| `health-checkup-guide-office-workers.md` | 健康/生活 | 微信公众号: 半佛仙人 |
| `reading-methodology-hecaitou-xiaolaituo.md` | 个人成长 | 微信公众号 |
| `social-auto-upload-github.md` | AI/技术 | GitHub: dreammis/social-auto-upload |
| `codex-beginner-tutorial-maynorai.md` | AI/技术 | GitHub: xianyu110/gpt-codex |
| `insurance-june-ranking-cross-analysis.md` | 投资理财 | 微信公众号: 深蓝保 |

---

## Inbox 近期新增（2026-05-29）

| 文件 | 分类 | 来源 |
|------|------|------|
| `codex-domestic-access-guide.md` | AI/技术 | 微信公众号 |
| `ruflo-agent-orchestration-platform.md` | AI/技术 | GitHub: ruvnet/ruflo |

---

## Inbox 历史新增（2026-05-28 及之前）

| 文件 | 分类 | 来源 |
|------|------|------|
| `ruflo-agent-orchestration-research.md` | AI/技术 | GitHub: ruvnet/ruflo |
| `hy-memory-tencent-research.md` | AI/技术 | memory.hunyuan.tencent.com |
| `ai-agent-repos-collection.md` | AI/技术 | GitHub: 0xsatoshis/ai-agent-repos-analysis |
| `ai-red-benefit-for-everyone-20260526.md` | AI/技术 | 微信公众号 |
| `schwab-account-opening-guide-innomad.md` | 投资理财 | X: @innomad_io |
| `personal-growth-productization-laoxue.md` | 个人成长 | 微信公众号: 老薛 |
| `agent-learning-roadmap-datawhale.md` | AI/技术 | GitHub: Datawhale |
| `easy-vibe-coding-guide.md` | AI/技术 | GitHub: Datawhale |
| `ppt-skills-collection.md` | AI/技术 | 用户分享 |
| `openwiki-knowledge-tool.md` | AI/技术 | 微信公众号 |
| `agent-harness-mini-structure.md` | AI/技术 | 微信公众号 |
| `investment-blogger-innomad.md` | 投资理财 | innomad.io |
| `us-etf-available-in-china.md` | 投资理财 | 微信截图 |
| `broker-comparison-schwab-vs-ibkr.md` | 投资理财 | X: @lzrationalnvest |
| `personal-broker-accounts-plan.md` | 投资理财 | 用户分享 |
| `schwab-deposit-guide-nicole.md` | 投资理财 | X: @Nicole_yang88 |
| `hongkong-sim-card-club-sim-guide.md` | 生活工具 | YouTube |

## 相关入口

- 全局规则：`../GLOBAL-RULES.md`
- Memory 索引：`../MEMORY.md`
- 框架总览：`../FRAMEWORK-README.md`

---

**最后更新**: 2026-05-30