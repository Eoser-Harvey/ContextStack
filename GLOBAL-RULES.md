...
**最后更新**: 2026-08-22
**版本**: v3.4
**变更**: ①自检清单扩至 8 项，新增「禁止推算、不确定必须问」（处理用户资金/持仓/成本/交易数据时绝不推算/反推/预测，数据不齐直接问，成本价只能以用户给的为准——2026-08-22 因华盛通CRCL加仓成本被推算，用户骂"胡扯"）；②同步记录此前已加入的「会话沉淀闭环（强制）」第7项；③协作模式新增「AI 行为约束自检清单」（6→8 项会话开始自检，对抗"加载规则 ≠ 遵守规则"漏洞，基于 4 次违规教训）
