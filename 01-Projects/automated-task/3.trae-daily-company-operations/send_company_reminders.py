...
SPECIAL_EVENTS_2026 = [
    # ✅ 2026-09-13 已完成，故从提醒中移除：
    #    · 补发 8 月工资 5,393.65 元（凭证 记-05，余额 → 19,586.91）
    #    · 首次个税实报（2026 年 08 月属期，本期收入 7,000 / 应补 0 / 申报成功无需缴款）
    {"date": "2026-09-26", "name": "工商公示实缴 3 万（截止）",
     "description": "gsxt.gov.cn，实缴后 20 日内；同时归档转账回单 + 出资证明。印花税 7.50 元（2027-01 按年缴）。"},
    {"date": "2026-09-30", "name": "残保金 + 公积金托收确认",
     "description": "① 登录电子税务局看「申报清册」有无残保金待办（30 人以下免征，无则不管）；② 公积金网厅确认 9 月托收是否成功。"},
    {"date": "2026-10-10", "name": "发 9 月工资 + Q3 绩效奖金 4,500 元",
     "description": "🔴 2026 年个税补救核心动作。应发 11,500（工资 7,000 + 奖金 4,500）− 社保公积金 1,606.35 − 个税 8.62 = **转账 9,885.03 元**，备注「2026年9月工资+Q3绩效奖金」。→ 完成后登记账簿 §0.5。"},
    {"date": "2026-10-15", "name": "个税实报（9 月属期）＋ Q3 季报 ＋ 财报报送（截止）",
     "description": "① 扣缴端申报 9 月属期：累计收入 18,500 → 应补 8.62 元 → **2026 年 10 月税款入库 ✅ 首次非零**；② Q3 季报（增值税+企税+附加税，共 6 项零申报）；③ 首次财务会计报表报送（资产负债表+利润表，取账簿 §7.1/§7.2）。"},
    {"date": "2026-11-15", "name": "个税实报（10 月属期）",
     "description": "累计收入 25,500 → 应补 11.81 元 → 2026 年 11 月税款入库 ✅。⚠️ 2027 年汇算不得申请退税（否则 2026 年记录被冲成 0）。"},
]
...
def get_all_deadlines():
    ...
    if dl_q and in_window(dl_q):
        ...
        deadlines.append((dl_q, f"Q{q_num}季度申报（6 项零申报）", "增值税(文化服务 3%)+企业所得税+城建税+教育费附加+地方教育附加。即使零收入也必须按期申报。"))
        # 🆕 2026-09-13 补充：财务会计报表报送（不是税种，故不在核定税种表里，极易漏）
        deadlines.append((dl_q, f"Q{q_num}财务会计报表报送", "报送资产负债表 + 利润表（数据直接取会计账簿 §7.1 / §7.2）。⚠️ 频率以电子税务局「申报清册」列示为准。"))

    # ===== 年度截止日 =====
    # 次年 5 月 31 日：企业所得税汇算清缴
    # 🔴 2026-09-13 修正：此前误写为 3 月 31 日 —— 那是「个人所得税·经营所得」的汇算期限，
    #    不适用于雇员工资薪金的公司。企业所得税为《企业所得税法》第 54 条"自年度终了之日起 5 个月内"。
    dl_531 = next_annual(5, 31)
    if in_window(dl_531):
        deadlines.append((dl_531, "企业所得税汇算清缴 + 财务报表（年度）", "上年度企业所得税多退少补，即使零收入也要申报；同时报送年度财务会计报表。漏报罚款 + 影响纳税信用。"))
