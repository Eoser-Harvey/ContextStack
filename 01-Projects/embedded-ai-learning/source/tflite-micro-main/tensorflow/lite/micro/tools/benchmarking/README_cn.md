...
如果模型被编译到工具中，则额外的模型分析信息会添加到元数据中。这包括模型内的数据使用情况、每个模型子图和推理执行顺序中的操作，以及模型中所有张量的信息。
...
该工具将输出所有输入张量的 CRC32，然后是 MicroInterpreter 预推理阶段的性能分析时间。接下来是每个算子的推理性能分析时间输出，以及所有推理操作的总结总计。最后输出所有输出张量的 CRC32 和 MicroInterpreter 竞技场内存使用情况。
...
使用编译到工具中的模型构建工具需要两个额外的 `make` 变量：
Building the tool with the model compiled in uses two additional `make`
variables:

* `GENERIC_BENCHMARK_MODEL_PATH`：TfLite 格式模型文件的路径。模型路径可以是绝对路径，也可以是相对于本地 TFLM 仓库的相对路径。此变量是必需的。
...
* `GENERIC_BENCHMARK_ARENA_SIZE`：TFLM 解释器竞技场的大小，以字节为单位。此变量是可选的。
...
当该工具与压缩模型一起使用时，将输出额外的性能分析时序。这将包括推理期间每个解压缩张量的性能分析时序，以及总结总计。虽然此性能分析时序是单独输出的，但解压缩的时序也包含在正常的推理性能分析时序和总结总计中。
...
张量解压缩操作可以在替代目标内存区域中发生。这允许使用专用内存作为解压缩数据的目标。该工具支持单个替代解压缩区域。使用以下 `make` 变量指定替代解压缩区域：
...
有关模型压缩的更多信息，请参阅[压缩文档](https://github.com/tensorflow/tflite-micro/blob/main/tensorflow/lite/micro/docs/compression.md)。
...
## 为 Cortex-M 使用 Corstone 300 模拟器构建和运行
...
此示例输出适用于使用 Corstone 300 的 Cortex-M：
