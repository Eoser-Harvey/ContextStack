...
> **日期**：2026-07-24（初建）→ 2026-08-11（核心完成）
> **状态**：✅ 核心完成（2026-08-04~08-11 AI 教学：CNN核心算子/架构选型/数据预处理/数据增强五件套/训练技巧三件套/优化器SGD vs Adam；CIFAR-10 CNN 实战模板未实操，作为对比案例讲解）
> **考核**：已考核（思考题 5 轮，易错点已沉淀 tech-interview-notes §8.7-8.9）
> **主线**：TFLM 源码阅读（conv.cc / pooling.cc / softmax.cc）
> **辅线**：CNN 架构、数据预处理、数据增强、训练技巧（围绕 TFLM 讲解）
> **源码根目录**：`source/tflite-micro-main/`
> **实践目标**：用 PyTorch 训练 CIFAR-10 CNN 模型（>70%），掌握数据工程全流程，为 Week 4 量化部署准备模型（未实操，仅概念）
