...
5. **同一模型 Keras(.h5) 和 TFLite(.tflite) 体积谁大？为什么？**
   ...
   >
   > 注：**降级算子 不等于 未使用算子**。降级算子指模型**已用到**、但 TFLM 无高效原生实现，被拆细或退化为 Flex/Select 的算子，会撑大体积甚至导致裸机跑不了；未使用的算子会被转换器剪掉（死代码消除），根本不进 `.tflite`，与"变大"无关。
...
> **量化边界澄清（易混点）：**
> - **Keras/TF → TFLite**：是 TF 框架基础转换（图优化 + 转 FlatBuffer）。**默认只是格式转换（float32→float32），量化是这同一步里的可选开关**（开 `optimizations` + 给校准集才量化成 int8），不是必然动作。
> - **TFLite → TFLM**：**不是量化**，是"转 C 数组 + MCU 解释器执行"。数值格式零变化（进来的 int8 就跑 int8，float32 就跑 float32），量化早在上一阶段固化进 `.tflite`，TFLM 只是照单执行。
> - **TFLM 不要求必须量化**：float32 也能跑，只是占 RAM/Flash 大、慢；量化是省资源的优化，非部署前提。
>
> | 阶段               | 真正发生的事                       | 量化在哪                         |
> |:-------------------|:-----------------------------------|:---------------------------------|
> | Keras/TF → TFLite  | 图优化（融合/折叠）+ 转 FlatBuffer   | **这里**（可选，转换期固化）       |
> | TFLite → TFLM      | 转 C 数组 + MCU 解释器执行          | **不在这一步**（零量化）           |
