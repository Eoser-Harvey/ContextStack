...
```
Stage 1 (r 向量)：
  L_max: 100-200ms | 5-10 FPS | mAP@0.5 > 60% | P_max < 15W

Stage 2：
  目标检测 → CNN → YOLO → Base: YOLOv8n

Stage 3：
  QAT INT8 → 剪枝 30% → 预训练+微调

Stage 4：
  NVIDIA Jetson Orin NX + TensorRT

Stage 5 (b 向量)：
  Latency: 12ms ✅  FPS: 30 ✅  Power: 12W ✅  mAP: 68% ✅
  → 全线满足，部署成功
```
